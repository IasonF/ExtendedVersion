...
$ 4 \times N \times 1.0E + 1 \times N \times 1.0E + 3 \times N \times 1.3E  + 4 \times N \times 0.3E + 4 \times N \times (0.3E + 0.9E) =  14.9NE $
...
The exploration of different options on memory organisation is performed using the steps described in Alg.~\ref{alg:clustering}. All potentially energy efficient configurations are tested for a given number of scenarios and the sequence of RTSs of the application. First, all the different configurations that satisfy the storage requirements of the most demanding RTS are constructed. Then, each configuration is tested for the sequence of RTSs and the one that minimizes Eq.\ref{eq:energy} is chosen as the most energy efficient.

\begin{algorithm}[!t]
\caption{Memory organisation exploration steps}
 \label{alg:clustering}
 \begin{algorithmic}[1]
		\STATE $RTS \gets$ storage requirement for all RTSs
		\STATE $Database \gets $ memory database
		\STATE $N \gets $ number of scenarios(up to 5 in this work)
			\FOR{$i = 1 \to N$}
			\STATE $Platform \gets $ i banks of Database
			\IF{$\sum_{1}^{i} size(bank) \leq  size(max(RTS))$} \STATE{Keep configuration} \ENDIF
			\STATE Find the configuration that minimizes Eq.\ref{eq:energy} for $RTS$			
			\ENDFOR
 \end{algorithmic}
\end{algorithm}
